"""Binary sensors for hass_juniper link state."""
...
class JuniperPortLinkBinarySensor(
    CoordinatorEntity[JuniperPortsCoordinator], BinarySensorEntity
):
    """Represent the operational link state of a Junos interface."""
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{entity_unique_id(host, interface)}::link"
    ...
    def name(self) -> str:
        ...
        if state is None or not state.description:
            return f"{self._interface} link"
        return f"{self._interface} - {state.description} link"

    @property
    def is_on(self) -> bool | None:
        """Return true when the interface operational state is up."""
        state = self.coordinator.data.get(self._interface)
        if state is None or state.oper_status is None:
            return None
        return state.oper_status.startswith("up")
...
async def async_setup_entry(
    ...
    def _add_new_interfaces() -> None:
        ...
        async_add_entities(
            [
                JuniperPortLinkBinarySensor(entry, coordinator, interface)
